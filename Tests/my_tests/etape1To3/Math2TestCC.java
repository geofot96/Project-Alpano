...
public class Math2TestCC {
    @Test
    public void sqSquaresRandomValues() {
        Random rng = TestRandomizerCC.newRandom();
        for (int i = 0; i < TestRandomizerCC.RANDOM_ITERATIONS; ++i) {
    ...
    public void firstIntervalContainingRootWorksOnSin() {
       double i1 = firstIntervalContainingRoot(new SinCC(), -1d, 1d, 0.1 + 1e-11);
        assertEquals(-0.1, i1, 1e-10);

        double i2 = firstIntervalContainingRoot(new SinCC(), 1, 4, 1);
        assertEquals(3, i2, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void improveRootFailsWhenIntervalDoesNotContainRoot() {
        improveRoot(new SinCC(), 1, 2, 1e-10);
    }

    @Test
    public void improveRootWorksOnSin() {
        double pi = improveRoot(new SinCC(), 3.1, 3.2, 1e-10);
        assertEquals(PI, pi, 1e-10);

        double mPi = improveRoot(new SinCC(), -4, -3.1, 1e-10);
    ...
    @Test
    public void firstIntervalContainingRootWorksOnDumbFunction() {
        double pi = firstIntervalContainingRoot(new dumbFunctionCC(), -20.05, 2.05, .1);
        assertEquals(-14.05, pi, 1e-10);
    }

}

class SinCC implements DoubleUnaryOperator {
    @Override
    public double applyAsDouble(double x) {
        return sin(x);
    }
}
class dumbFunctionCC implements DoubleUnaryOperator{
    @Override
    public double applyAsDouble(double x) {
        return 2*x+28;
    }
